module.exports = (client, member) => {
    ...
    const content = db.fetch(`sunucular.${member.guild.id}.gelen.content`)
	if(channel){
    ...
    const role = db.fetch(`sunucular.${member.guild.id}.otorol.rol`)
    const statusrole = db.fetch(`sunucular.${member.guild.id}.otorol.status`)
    if(role){
